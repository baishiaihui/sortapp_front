<template>
  <div class="add-form">
    <b-button class="to_edit_button" @click="to_edit()" variant="success"
      >検索画面へ</b-button
    >
    <h3>変更</h3>
    <b-form @submit.prevent="login()">
      ...
      <b-button variant="success" @click="update()" class="create-button"
        >登録</b-button
      >
    </b-form>
  </div>
    ...
    update: async function () {
      if (this.$v.$invalid) {
        ...
        const res = await this.$axios.$put(
          `/api/v1/sort_infos/${this.$route.params.id}`,
          this.sortinfo
        );

        if ((res.status = "SUCCESS")) {
          this.$toast.success("更新に成功しました。", {
            position: "top-center",
          });
        } else {
